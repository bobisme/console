...
fn poke_rows_overwrites_the_region() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,2", "--rows", "01020304,0a0b0c0d"]));
    assert_eq!(code, 0);
...
fn poke_only_rewrites_rows_that_actually_changed() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,2,4", "--rows", &new_rows.join(",")]));
    assert_eq!(code, 0);
...
fn poke_identical_rows_is_a_legal_noop() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,1", "--rows", "a0000000"]));
    assert_eq!(code, 0);
...
fn poke_dry_run_leaves_the_file_untouched_and_prints_report() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,1", "--rows", "a0000000", "--dry-run"]));
    assert_eq!(code, 0);
...
fn poke_creates_the_map_section_when_absent() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,2,1", "--rows", "1f2a"]));
    assert_eq!(code, 0);

    let out = read(&path);
    assert!(out.contains("__map__"), "poke must create the section: {out}");
...
fn poke_wrong_row_count_errors_without_modifying_file() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,2", "--rows", "01020304"]));
    assert_ne!(code, 0);
...
fn poke_wrong_row_length_errors_naming_expected_and_got() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,1", "--rows", "abcd"]));
    assert_ne!(code, 0);
...
fn poke_bad_hex_char_errors_without_modifying_file() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,1", "--rows", "00zz0000"]));
    assert_ne!(code, 0);
    assert_eq!(read(&path), before, "file must be untouched on error");
}
...
fn poke_out_of_bounds_region_errors_without_modifying_file() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "127,0,4,1", "--rows", "01020304"]));
    assert_ne!(code, 0);
    assert_eq!(read(&path), before);
}
...
fn poke_rejects_both_rows_and_stdin() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,1", "--rows", "00000000", "--stdin"]));
    assert_ne!(code, 0);
    assert_eq!(read(&path), before);
}
...
fn dump_then_poke_is_a_noop() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,3", "--rows", &dumped_rows.join(",")]));
    assert_eq!(code, 0);
...
fn poke_then_dump_round_trips() {
    ...
    let code = cli_poke(&args(&[path.to_str().unwrap(), "0,0,4,2", "--rows", "11223344,55667788"]));
    assert_eq!(code, 0);
